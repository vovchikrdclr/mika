define(['dispatcher', 'scroll/scroll.store', 'resize/resize.store', 'utils'], function(dispatcher, scrollStore, resizeStore, utils) {
	...
	var el = document.getElementsByClassName('inner-text text1')[0];
	var block = document.getElementsByClassName('block2')[0];
	var showBlock;
	var hiddenBlock;
	...
	var _handleChange = function() {
		...
		if (storeData.top > showBlock && storeData.top < hiddenBlock && firstChange === false) {
			...
			firstChange = true;
		} else if (storeData.top <= showBlock && firstChange === true || storeData.top >= hiddenBlock && firstChange === true) {
